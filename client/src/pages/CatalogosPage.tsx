...
function CatalogTable({ title, items, isLoading, isAdmin, onCreate, onUpdate, onDelete }: any) {
  ...
  return (
    <div className="space-y-3">
      ...
      <DataTable columns={columns} data={items || []} isLoading={isLoading} emptyMessage={`Sin ${title.toLowerCase()}`} rowKey={(i) => i.id} />
      <Dialog open={formOpen} onOpenChange={setFormOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>{editing ? `Editar` : `Nuevo`} {title.slice(0, -1).toLowerCase()}</DialogTitle></DialogHeader>
...
export default function CatalogosPage() {
  ...
  return (
    <div className="space-y-8">
      <PageHeader title="Catalogos" description="Aceites y consumibles" />
      <CatalogTable
        title="Aceites"
        items={aceites}
