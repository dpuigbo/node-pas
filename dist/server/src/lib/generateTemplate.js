...
function ejesDeCinematica(cinematica) {
    if (!cinematica)
        return 6;
    if (cinematica.includes('gantry'))
        return 4; // portico X/Y/Z (+C opcional), p.ej. IRB 840A
...
const tituloGrupoEjes = (g) => g.length === 1 ? `Control eje ${g[0]}` : `Control ejes ${g.slice(0, -1).join(', ')} y ${g[g.length - 1]}`;
...
function fieldsTable(key, title, fields) {
    return block('table', {
        key, label: '', title, ...TBL,
...
function buildMechanicalSchema(input) {
    ...
    const cal = calibracionBlock(profile.calibracion, nEjes, profile.calibracion === 'kuka_angulo'
...
function buildControllerSchema(input) {
    ...
    const valorLabels = ['Versión del sistema'];
    if (profile.sistemaConRam)
        valorLabels.push('Disponibilidad de la RAM', 'Ocupación de la RAM');
    const checkLabels = [];
    if (profile.sistemaConRam)
        checkLabels.push('Clonado de disco duro');
...
async function overhaulForModel(prisma, modeloId) {
    const acts = await prisma.actividadPreventiva.findMany({
        where: { tipoActividad: { codigo: 'overhaul' } },
        select: { modelosAplicables: true, intervaloHoras: true },
    });
    for (const a of acts) {
        if ((0, planMantenimiento_1.actividadAplicaAModelo)(a, modeloId) && a.intervaloHoras)
            return Number(a.intervaloHoras);
    }
    return null;
}
