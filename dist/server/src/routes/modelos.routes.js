...
router.get('/compatible-con', async (req, res, next) => {
    try {
        ...
        if (tipo === 'mechanical_unit') {
            ...
            const familiasOk = await database_1.prisma.compatibilidadRobotControlador.findMany({
...
router.put('/:id/lubricacion/:lubId', (0, role_middleware_1.requireRole)('admin'), async (req, res, next) => {
    try {
        ...
        const legacy = await database_1.prisma.lubricacionReductora.findUnique({ where: { id: legacyId } });
        if (!legacy) {
        ...
        const existing = await database_1.prisma.lubricacion.findFirst({
        ...
        let row;
        if (existing) {
            row = await database_1.prisma.lubricacion.update({
            ...
            row = await database_1.prisma.lubricacion.create({
                data: dataPayload,
                include: {
                    aceite: { select: { id: true, nombre: true, fabricante: true } },
                    consumible: { select: { id: true, nombre: true, tipo: true, unidad: true } },
...
router.put('/:id/compatibilidad', (0, role_middleware_1.requireRole)('admin'), async (req, res, next) => {
    try {
        ...
        await database_1.prisma.$transaction([
            database_1.prisma.compatibilidadControlador.deleteMany({ where: { componenteId: id } }),
            ...(controladorIds.length > 0
                ? [database_1.prisma.modeloComponente.update({
                        where: { id },
                        data: {
                            controladoresCompatibles: {
                                create: controladorIds.map((cId) => ({ controladorId: cId })),
        ...
        const updated = await database_1.prisma.modeloComponente.findUnique({
            where: { id },
            include: {
                controladoresCompatibles: {
                    include: { controlador: { select: { id: true, nombre: true } } },
