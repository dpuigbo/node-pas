...
const MERGE_GRUPOS: { ids: number[]; renameTo?: string }[] = [
...
const MERGE_PREFIX: { actId: number; dropConsumibleId: number }[] = [
...
router.post('/merge-grupos', adminMiddleware, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const apply = req.body?.apply === true;
    ...
    })) as unknown as { id: number; componente: string; modelosAplicables: unknown; consumibles: { id: number; consumibleId: number }[] }[];
    const byId = new Map(acts.map((a) => [a.id, a]));

    const plan = MERGE_GRUPOS.map((g) => {
      const members = g.ids.map((id) => byId.get(id)).filter(Boolean) as typeof acts;
    ...
    writeFileSync(backupPath, JSON.stringify({ acts, grupos: MERGE_GRUPOS, prefix: MERGE_PREFIX }));
    ...
    for (const f of MERGE_PREFIX) {
      ...
      for (const c of a.consumibles.filter((x) => x.consumibleId === f.dropConsumibleId)) {
        await prisma.actividadConsumible.delete({ where: { id: c.id } });
    ...
    for (const p of plan) {
      ...
      if (p.grupo.renameTo) renombradas++;
      const canonCons = new Set(p.canon.consumibles.map((c) => c.consumibleId));
      for (const d of p.dups) {
        for (const dc of d.consumibles) {
          if (canonCons.has(dc.consumibleId)) { await prisma.actividadConsumible.delete({ where: { id: dc.id } }); consBorrados++; }
